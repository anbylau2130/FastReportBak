...
namespace USP.Common
{
    ...
    public class JsonChecker
    {
        ...
        public static string Querystring2Json(string str)
        {
            List<string> @params = str.Split('&').ToList();
            ...
            {
                if (@params[index].Split('=').ToList().Count == 2)
                {
                    jsonresult.Append(@params[index].Split('=').ToList()[0]);
                    jsonresult.Append(":");
                    if (@params[index].Split('=').ToList()[1].IsBool() ||
                        @params[index].Split('=').ToList()[1].IsFloat() || 
                        @params[index].Split('=').ToList()[1].IsInt()|| 
                        @params[index].Split('=').ToList()[1].IsDecimal())
                    {
                        jsonresult.Append(@params[index].Split('=').ToList()[1]);
                    }
                    else
                    {
                        jsonresult.Append("'").Append(@params[index].Split('=').ToList()[1]).Append("'");
                    }
                    
                    jsonresult.Append(index != @params.Count-1 ? "," : string.Empty);
                }
            }
            jsonresult.Append("}");
